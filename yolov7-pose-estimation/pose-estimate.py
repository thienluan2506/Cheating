...
def run(
    ...
    out_video_name = f"{video_path.split('/')[-1].split('.')[0].split('_')[0]}"
    ...
    while(cap.isOpened):
        ...
        if ret:
            ...
            for idx in range(output.shape[0]):
                ...
                if len(Id_person) <= idx:
                  Id_person.append(box)
                  continue
                if len(Id_person) > output.shape[0]:
                    Id_person.pop(-1)
                for i in range(len(Id_person)):
                  if IOU(Id_person[i],box) > 0.7:
                    
                    if i==0:
                    # if i!=None:
                      Array.append(array_point)
                      cv2.putText(im0, str(i), (int(xmin), int(ymin)), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, 2)
                ...
                strID = str(idx)
                # cv2.circle(im0, (int(output[idx, 2]),int(output[idx, 3])), 1, (0, 0, 255), 2)
                # convert pose conlletion
                # x_obj , y_obj = (766, 382) # object Tri
                x_obj , y_obj = (596,499) # object Minh right
                # x_obj , y_obj = (356,475) # object Minh
                ...
                #     cv2.putText(im0, strID, (int(xmin), int(ymin)), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, 2)
                #     cv2.rectangle(im0,(int(xmin), int(ymin)),(int(xmax), int(ymax)),color=(255, 0, 0),
                #     thickness=1,lineType=cv2.LINE_AA)
                #     array_point = remove(array_point)
                #     a.append(array_point)
                # else:
                #   continue
            #Calculatio for FPS
            end_time = time.time()
    ...
    print(Array)
    # pick label
    res = "".join(filter(lambda x: not x.isdigit(), out_video_name))
    print(res)
    #########
    FinalVal = ConvertToDataFrame(Array, res)
    # FinalVal.to_csv('Dataset.csv', index =False) # begin add keypoint into .csv
    FinalVal.to_csv('Non-Cheating.csv', mode = 'a', index =False, header = False) # append filr .csv 
